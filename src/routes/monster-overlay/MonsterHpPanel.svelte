<script lang="ts">
  ...
  const sections = $derived(monsterHpSections());
  const styleConfig = $derived(hpPanelStyle());
  const panelPos = $derived(getHpPanelPosition());
  const panelScale = $derived(getHpPanelScale());
</script>

{#if sections.length > 0 || scaffold}
  <div
    class="overlay-group monster-hp-panel"
    ...
    onpointerdown={(event) =>
      startMonsterDrag(event, { kind: "hpPanel" }, panelPos)}
  >
    {#if scaffold}
      <div class="group-tag">{t("monsterOverlay.hpGroupTag")}</div>
    {/if}
      ...
      {#each sections as section (section.bossEntityUuid)}
        <section class="boss-section" class:placeholder={section.isPlaceholder}>
          <div class="boss-title">
            {t("monsterOverlay.hpSectionTitle", { title: section.title })}
          </div>
    ...
    {#if editing}
      <div
        class="resize-handle"
        onpointerdown={(event) =>
          startMonsterResize(event, { kind: "hpPanel" }, panelScale)}
      ></div>
    {/if}
  </div>
{/if}
...
<style>
  ...
  .monster-hp-panel.has-background {
    padding: 6px;
    border-radius: 10px;
    border: 1px solid rgba(148, 163, 184, 0.24);
  }

  .monster-hp-panel.editable {
